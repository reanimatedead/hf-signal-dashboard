/* flow.js — お金の流れ Canvasアニメ (data.json.flow 駆動) */
window.HFFlow = (function () {
  ...
  function draw() {
    ...
    basins.forEach(b => {
      ...
      for (let i = 0; i <= BW; i += 6) { const yy = y + BH - wh + Math.sin(i * .12 + Date.now() * .004) * 1.6; i ? ctx.lineTo(x + i, yy) : ctx.moveTo(x + i, yy); }
    ...
    for (const p of P) { ctx.fillStyle = p.target === "_cb" ? "#39d0d8" : "rgba(120,170,235,.85)"; ctx.beginPath(); ctx.arc(p.x, p.y, 1.8, 0, 7); ctx.fill(); }
